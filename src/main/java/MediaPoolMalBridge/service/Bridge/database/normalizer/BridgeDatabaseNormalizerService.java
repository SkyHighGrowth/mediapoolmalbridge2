...
public class BridgeDatabaseNormalizerService extends AbstractService {
    ...
    /*@Transactional
    protected void transferJsonedValues() {
        final Iterable<AssetEntity> assets = assetRepository.findAll();

        for( final AssetEntity assetEntity : assets ) {
            final AssetJsonedValuesEntity assetJsonedValuesEntity = new AssetJsonedValuesEntity();
            assetJsonedValuesEntity.setBmUploadMetadataArgumentJson( assetEntity.getBmUploadMetadataArgumentJson() );
            assetJsonedValuesEntity.setMalGetAssetJson( assetEntity.getMalGetAssetJson() );
            assetJsonedValuesEntity.setMalGetUnavailableAssetJson( assetEntity.getMalGetUnavailableAssetJson() );
            assetJsonedValuesRepository.save( assetJsonedValuesEntity );
