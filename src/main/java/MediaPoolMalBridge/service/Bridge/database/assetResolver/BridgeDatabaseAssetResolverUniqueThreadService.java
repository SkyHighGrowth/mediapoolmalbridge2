...
public class BridgeDatabaseAssetResolverUniqueThreadService extends AbstractBridgeUniqueThreadService {
    ...
    protected void run() {
        for( int page = 0; true; ++page ) {
            ...
            if( assetEntities.isEmpty() || page > 500 ) {
                break;
            }
    ...
    protected void resolvePage( final List<AssetEntity> assetEntities ) {
        assetEntities.forEach(assetEntity -> {
            final String message = String.format( "Asset with id [%s] transfer ended in status [%s] and total transfer has not finished  succesfully", assetEntity.getMalAssetId(), assetEntity.getTransferringAssetStatus().name());
